...
document.addEventListener('DOMContentLoaded', async (): Promise<void> => {
    ...
    const copyUnknownPromptBtn = document.getElementById('copy-unknown-prompt-btn') as HTMLButtonElement;
    ...
    const syncDeleteAccountBtn = document.getElementById('sync-delete-account-btn') as HTMLButtonElement;
    ...
    addWordSubmit.addEventListener('click', async (): Promise<void> => {
        const word: WordItem = {
    ...
    sortSelect.addEventListener('change', async (): Promise<void> => {
        await dictionary.setSortMode(sortSelect.value as DictionarySortMode);
        refreshWordbook();
        scheduleAutoSave();
    });
    ...
    if (copyUnknownPromptBtn) {
        copyUnknownPromptBtn.addEventListener('click', (): void => {
...
2. N3~N2 정도의 난이도로 작성해줘.`;
    ...
    syncDeleteAccountBtn.addEventListener('click', (): void => {
    ...
    async function removeWord(text: string): Promise<void> {
        if (!(await dictionary.remove(text))) return;
        refreshWordbook();
    ...
    function askAndSave(word: WordItem): void {
        ...
        const saveDialog = createSaveDialog(word, async (finalWord: WordItem) => {
            if (await dictionary.add(finalWord)) {
                ...
                if (latestJsonWords.length > 0) {
                    ...
                    const safeWordsArray = Array.isArray(currentWords) 
                        ? currentWords 
                        : (currentWords && typeof currentWords === 'object' && 'list' in currentWords ? (currentWords as any).list : []);
        ...
        try {
            ...
            if (latestJsonWords.length > 0) {
                ...
                const safeSavedWords = Array.isArray(allData) 
                    ? allData 
                    : (allData && typeof allData === 'object' && 'list' in allData ? (allData as any).list : []);
            ...
            if (error?.message === 'invalid_data' || error?.toString().includes('invalid_data')) {
                ...
                try {
                    const fallbackMode = sortSelect.options[0]?.value as DictionarySortMode;
                    if (fallbackMode) {
    ...
    async function deleteAccountFromDialog(): Promise<void> {
        if (!requireSyncSession() || !syncSession) return;
        if (autoSaveTimer) {
            window.clearTimeout(autoSaveTimer);
            autoSaveTimer = undefined;
        }
...
function createAccountDeleteDialog(onConfirm: () => void): {
    open: () => void;
    close: () => void;
} {
    const dialog = document.createElement('div');
...
function createLogoutConfirmDialog(onConfirm: () => void): {
    open: () => void;
    close: () => void;
} {
    const dialog = document.createElement('div');
